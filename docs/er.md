...
| manager_user_id | 管理者 ID          | INTEGER  | 1                   |     | ○   |              |                |
...
| created_at      | データ作成日時     | DATETIME | 2023-04-02 10:00:00 |     |     |              |                |
| updated_at      | データ更新日時     | DATETIME | 2023-05-01 09:00:00 |     |     |              |                |
...
| event_at     | 変更日時          | DATETIME | 2023-05-01 10:00:00 |     |     |              |                |
...
erDiagram
    ...
    EQUIPMENTS {
        ...
        CHAR category FK "-> equipment_categories.code"
        INT category_index "研究室内の管理インデックス"
        VARCHAR name "物品名，型番"
        VARCHAR type "種別"
        INTEGER user_id FK "-> users.user_id (使用者)"
        INTEGER manager_user_id FK "-> users.user_id (管理者, 社内)"
        ...
        ENUM status "物品の貸し出し状態．available, borrowed, broken, disposed"
        TEXT remarks "備考"
    ...
    EQUIPMENT_HISTORY {
        ...
        INTEGER equipment_id FK "-> equipments.equipment_id"
        VARCHAR event_type "assign/return/status_change/..."
        ENUM old_value "前の状態"
        ENUM new_value "新しい状態"
        INTEGER changed_by FK "-> users.user_id (操作した人)"
        DATETIME event_at
    }
